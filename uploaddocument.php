      ...
      if( $extension == $size<=$max_size){
        ...
        if(move_uploaded_file($tmp_name, $location.$name)){
          if($_SESSION['loginas']=="tutor"){
            $query = "INSERT INTO `tutorupload` (tutorid,title,description,filename, filetype, location,uploaddate) VALUES ($userid,'$title','$description','$name', '$type', '$location$name','$uploaddate')";
          }
          if($_SESSION['loginas']=="student"){
            $query = "INSERT INTO `upload` (studentid,title,description,filename, filetype, location,uploaddate) VALUES ($userid,'$title','$description','$name', '$type', '$location$name','$uploaddate')";
          }
         
                $result = mysqli_query($link, $query);

                if ($result) {
                  echo "<script type='text/javascript'>if(confirm('Document Uploaded!')) 
                  { window.open('uploaddocument.php','_self');} else {window.open('uploaddocument.php','_self');}</script>";
              ...
              <div class="card shadow mb-4">
                ...
                <form action="" method="POST" enctype="multipart/form-data">
                  <div class="form-group">
                    <label for="tutor">Document Title:</label> 
                ...
                <div class="form-group">
                  ...
                  <p class="help-block">Upload Files that are below 10 MB and file types .pdf,.docx,.png,.jpeg,.jpg</p>
                </div>
                  ...
                  <div class="form-group">
                    <button name="submit" name="send" type="submit" class="btn btn-primary">UPLOAD</button>
                  </div>
                </form>
